<?php session_start(); include "../conex.php";
	...
	$consulta = "SELECT * FROM  productos_imagenes where id_producto=".$_SESSION["id_prod"]." ORDER BY orden";
	$resultado = mysqli_query($link, $consulta);
	$elementos = null;
	while ($datos = mysqli_fetch_assoc($resultado)){
		$elementos[$datos['id']] = $datos['foto'];
	}
...
<head>
	<meta charset="utf-8" />
	<title>Productos Imagenes ordenar</title>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class="active">Productos Imágenes [Ordenar]</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
						...
						<ul id="lista" class="ui-sortable row" style=" width:auto; margin:auto">
								<?php 
                                    foreach ($elementos as $id => $nombre)
                                        echo '<li class="col-md-2" id="elemento-'.$id.'" contenteditable="false"><b><img style="width:100px" src="../items/'.$nombre.'"></b></li>';
                                ?>
						</ul>
		...
		$(function(){
			...
			lista.sortable({
                update: function(event,ui){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'productos-imagenes-controlador.php',
                        dataType: 'json',
                        data: {
                            accion: 'ordenar',
                            puntos: ordenPuntos
