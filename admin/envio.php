...
if(isset($_GET["eliminar"])){
	eliminar('envio',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
		$update=mysqli_query($link,"update envio    set 
		 estatus='".$estatus."',
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update envio set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into envio values(NULL,NULL,NULL,NULL,NULL,NULL)");
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update envio set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="envio-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			
		
			&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="envio-ordenar.php" class="btn btn-default btn-lg">Ordenar <i class="ti-move"></i></a>
			<br><br>
			<!-- BEGIN table -->
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="white-space: nowrap">envio</th>
...
?>
					...
					<tr>
						<td><?=$row1["id"]?></td>
						<td><?=strtoupper($row1["nombre_es"])?></td>
...
<a href="envio-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
