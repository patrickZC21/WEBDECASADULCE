...
if(isset($_GET["eliminar"])){
	eliminar('faq',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
		$update=mysqli_query($link,"update faq    set 
		 estatus='".$estatus."',
		 titulo='".limpiar($link,$_POST["titulo"])."' 	,
		  texto='".limpiar($link,$_POST["texto"])."'
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into faq values(NULL,NULL,NULL,NULL)");
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class="active">Preguntas Frecuentes (FAQ)</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<!-- END page-header -->
			<a href="faq-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="width:1%"></th>
						 <th class="no-sort" style="white-space: nowrap">Pregunta</th>
						<th class="no-sort" style="white-space: nowrap">Respuesta</th>
...
$sql1=mysqli_query($link,"select * from  faq where estatus='si' ");
...
<a href="faq-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
<a onClick=" if(confirm('Esta seguro?')){window.location='<?=$_SERVER["PHP_SELF"]?>?eliminar=<?=$row1["id"]?>'};"  class='btn btn-default btn-xs'><i class="ti-close"></i></a>
						...
						<td><?=ucwords(strtolower($row1["titulo"]))?></td>
					<td><?=ucwords(strtolower($row1["texto"]))?></td>
