...
if(isset($_GET["id"])){$_SESSION["id_item"]=$_GET["id"];}

//E L I M I N A R
if(isset($_GET["eliminar"])){
	eliminar('tallas',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
		$update=mysqli_query($link,"update tallas    set 
		 estatus='".$estatus."',
		 talla='".$_POST["talla"]."',
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into tallas values(NULL,NULL,NULL,NULL,NULL)");
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class=""><a href="items.php">Items</a></li>
				<li class="active">Tallas - Items</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<!-- END page-header -->
			<a href="items-tallas-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="white-space: nowrap">Talla</th>
...
$sql1=mysqli_query($link,"select * from  tallas where id_item=".$_SESSION["id_item"]." order by orden ASC");
$y=1; while($row1=mysqli_fetch_array($sql1)){ 

$item=consultar("select nombre_es from items where id=".$row1["id_item"]."",$link);
...
<a href="items-tallas-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
