...
$consulta=mysqli_fetch_array(mysqli_query($link,"select * from categorias where id=".$_GET["id"].""));
...
<head>
	<meta charset="utf-8" />
	<title>SubCategorías editar</title>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class="active">SubCategorías [Editar]</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<div id="rootwizard" class="wizard wizard-full-width">
				...
				<div class="wizard-header">
					<ul class="nav nav-pills">
						<li class="active"><a href="#tab1" data-toggle="tab">SubCategorías - EDITAR</a></li>
								
					</ul>
				</div>
				
<form action="subcategorias.php" method="post" enctype="multipart/form-data" onSubmit="$('#btn_editar').html('Editando ...')" name="wizard_form">
...
Seleccione categoria : 
									<select name="id_categoria" class="form-control">
										...
										<?php  $con = mysqli_query($link, "select * from categorias where estatus='si'");
												while($row=mysqli_fetch_assoc($con){
													?>
													
<option value="<?=$row["id"]?>"  <?php  if ($consulta["id_categoria"]==$row["id_categoria"]){ echo "selected";} ?>><?=$row["nombre_es"]?></option>
