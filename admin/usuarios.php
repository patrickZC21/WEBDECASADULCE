...
if (isset($_POST["btn_editar"])){
		...
		$update=mysqli_query($link,"update _usuarios set 
		login='".$_POST["login"]."',
...
if (isset($_POST["btn_clave"])){
	...
	//consulto con la tabla _usuarios si existe esta clave
	$query = mysqli_query($link,"SELECT * FROM `_usuarios` WHERE  clave='".$_POST["clave"]."' and id=".$_POST["id"]." ") or die(mysqli_error($link));
	
	$filas=mysqli_num_rows($query);
	
	if ($filas>0){
		// SI ES CORRECTA PROCEDO A CAMBIAR LA CLAVE
		      mysqli_query($link,"update _usuarios set  clave='".$_POST["nclave"]."' where id=".$_POST["id"]." ");		
		     $hide2="";
		}else{
			$hide="";
...
if (isset($_POST["btn_guardar"])){
		...
		$update=mysqli_query($link,"insert into  _usuarios(nombres,login,clave,estatus,tipo_usuario,email) values ( 
		'".mysqli_real_escape_string($link,$_POST["nombres"])."',
		'".mysqli_real_escape_string($link,$_POST["login"])."',
		'".mysqli_real_escape_string($link,$_POST["clave"])."',
		'activo',
		'administrador',
		'".mysqli_real_escape_string($link,$_POST["email"])."'
		)") or die ("Error ingresar");	
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			 <?php  if ($_SESSION['ADM_tipo_usuario']=="administrador"){ ?> 
                        <a href="usuarios-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a> 
                          <?php   }   ?> 
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						...
						<th class="no-sort" style="width:1%"></th>
						<th class="no-sort" style="white-space: nowrap">Login</th>
				...
				<tbody>
				 ...
				 <?php 
								if ($_SESSION['ADM_tipo_usuario']=="administrador"){
									 $sql1=mysqli_query($link,"select * from  _usuarios");
									// echo "select * from  _usuarios";
									 
									}else{
									 $sql1=mysqli_query($link,"select * from  _usuarios_ where id=".$_SESSION["id_usuario_ADM"]." and estatus='activo'");
									 // echo "select * from  _usuarios where id=".$_SESSION["id_usuario_ADM"]." and estatus='activo'";	
										}
					...
					<tr>
						  ...
						  <td class="btn-col" style="white-space: nowrap">
							...
							<button onClick="window.location='usuarios-editar.php?id=<?=$row1["id"]?>'"  class='btn btn-success btn-xs'>Editar</button>
                                           
                                            <button  onClick="window.location='usuarios-cambio-clave.php?id=<?=$row1["id"]?>'"   class='btn btn-info btn-xs' >Cambio Clave</button>
                                                     ...
                                                     <td  ><?php echo $row1["login"]?></td>
                                                      ...
                                                      <td  ><?php echo $row1["nombres"]?></td>
                                                       <td  ><?php echo $row1["email"]?></td>
                                                          ...
                                                          <td  ><?php if ($row1["estatus"]=="activo"){echo "<i style='color:#72c02c;background-color:#72c02c' class='ti-control-record'></i>";}
														  else{echo"<i style='color:#F93;background-color:#F93' class='ti-control-record'></i>"; }?></td>
                                                     
                                                       <td  ><?php echo $row1["tipo_usuario"]?></td>		 
